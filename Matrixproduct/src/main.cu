...
// host implementation
extern void convSeparableHost( float* kdata, const int& kernel_supp, const Image& image, Image& image_conv);
...
__global__ void convSeparable(float* kernel, float* image, float* image_conv, const unsigned int image_size) {
	...
	KernelSeperable< kernel_supp_half > kernelmapper = (KernelSeperable<kernel_supp_half>*) &weights;
	Kernelmapper->apply(pixel_x, pixel_y, image, image_conv, image_size);

#if 0
	float val = 0.0;
	int iky = 0;
	int ikx = 0;
	for (int iy = pixel_y - kernel_supp_half; iy <= pixel_y + kernel_supp_half; ++iy, iky++)
	{
		float wy = kernel[iky];
		for (int ix = pixel_x - kernel_supp_half; ix <= pixel_x + kernel_supp_half; ++ix, ikx++)
		{
			if (ix >= 0 && ix <= image_size && iy >= 0 && iy < image_size)
			{
				float wx = kernel[ikx];
				val += wx *wy * image[iy*image_size + ix];
			}
		}
		ikx = 0;
	}

	image_conv[pixel_y*image_size + pixel_x] = val;
#endif
}
...
int main( int /*argc*/, char** /*argv*/ ) {
  ...
  if (0 != image.n_cols % max_threads_per_block_sqrt)
  {
	  num_blocks.x++;
  }
  
  num_blocks.y = image.n_rows / max_threads_per_block_sqrt;
  if(0 != image.n_rows % max_threads_per_block_sqrt)
  {
	  num_blocks.y++;
  }
  ...
  convSeparable<kernel_supp_half><<< num_blocks , num_threads_per_block >>>( kernel_device,
